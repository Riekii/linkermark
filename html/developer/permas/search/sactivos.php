...
EXTRACT(YEAR_MONTH FROM max(mensajes.datetime)) DESC,
EXTRACT(DAY FROM max(mensajes.datetime)) DESC,
EXTRACT(HOUR FROM max(mensajes.datetime)) DESC
LIMIT 15";
...
while($rowm = $conmen2->fetch_assoc()){
    ...
    echo '
        <tr class="stimeline" style="text-align:center;">
